...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* sidebar */}
      <aside
        className={`fixed top-0 left-0 h-screen w-64 bg-card border-r border-border p-5 flex flex-col z-50 transform transition-transform duration-300 md:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}
      >
          ...
          {links.map((link) => {
            ...
            return (
              <Link
                ...
                key={link.name}
                onClick={() => setOpen(false)}
                className={`flex items-center gap-3 px-4 py-6 rounded-lg text-lg transition ${isActive ? 'bg-primary text-white' : 'text-gray-400 hover:bg-surface hover:text-white'}`}
              >
                <Icon size={18} />
                {link.name}
              </Link>
